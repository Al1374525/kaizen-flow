...
export function calculateXP(input: XPCalculationInput): XPCalculationResult {
  ...
  const actionMultiplier =
    XP_CONSTANTS.ACTION_MULTIPLIERS[action.actionType] || 1.0;
  ...
  const totalMultiplier =
    avoidanceMultiplier * difficultyMultiplier * actionMultiplier;
...
export function calculatePhoenixPoints(daysSinceLastAction: number): number {
  if (daysSinceLastAction >= 30)
    return XP_CONSTANTS.PHOENIX_POINT_THRESHOLDS['30d'];
  if (daysSinceLastAction >= 14)
    return XP_CONSTANTS.PHOENIX_POINT_THRESHOLDS['14d'];
  if (daysSinceLastAction >= 7)
    return XP_CONSTANTS.PHOENIX_POINT_THRESHOLDS['7d'];
  if (daysSinceLastAction >= 3)
    return XP_CONSTANTS.PHOENIX_POINT_THRESHOLDS['3d'];
  return 0;
}
